...
//  ok( "post resource returns location (new slug generated)", slug !== cfg.r1name && slug.endsWith('-test1.ttl'),res)
  ...
  ok("200 patch sparql insert, delete to existing resource",res.status==200 && await res1.text() === cfg.resPatchSparql , res1)

  res = await PATCH( cfg.file1,cfg.patchN3_1, 'text/n3' )
  res1 = await GET( cfg.file1 )
  ok("200 patch n3 insert",res.status==200 && await res1.text() === cfg.resPatchN3_1, res1)

  res = await PATCH( cfg.file1,cfg.patchN3_3, 'text/n3' )
  res1 = await GET( cfg.file1 )
  ok("200 patch n3 delete, insert, where",res.status==200 && await res1.text() === cfg.resPatchN3_2, res1)
